(function () {
  ...
  function render(rows) {
    ...
    rows.forEach(row => {
      ...
      const item = players.get(player) || { player, entries: 0, score: 0, raw: 0, places: [0, 0, 0, 0] };
      item.entries += 1;
    ...
    playersArea.innerHTML = `<table class="qualifying-detail-table"><thead><tr><th>選手</th><th>出場</th><th>ポイント</th><th>素点合計</th><th>1-2-3-4着</th></tr></thead><tbody>${playerRows.map(item => `<tr><td>${escapeHtml(item.player)}</td><td>${item.entries}</td><td class="${item.score < 0 ? "qualifying-score-negative" : "qualifying-score-positive"}">${formatScore(item.score)}</td><td>${item.raw.toLocaleString("ja-JP")}</td><td>${item.places.join("-")}</td></tr>`).join("")}</tbody></table>`;
