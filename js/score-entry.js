(function(){
  ...
  async function init(){sessionStorage.removeItem("hldbScoreAdminMode");inputDate=timedInputDate();await loadReferenceData();try{await loadServerEntries()}catch(error){console.warn(error)}$("listDate").value=today();updateAdminButton();setupInputPreferences();startInputDateTimer()}
  init();
})();
