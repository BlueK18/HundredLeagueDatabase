(function(){
  ...
  function refreshInputSelectors(rebuild=true){
    ...
    $("inputInfoBanner").textContent=DEMO_MODE?`${date.replaceAll("-","/")} の予定表を使って練習しています`:waitingForNoon?"本日の入力フォームは12時に自動で切り替わります":date===calendarToday?"予定表から本日のチームを自動表示します":`${date.replaceAll("-","/")} 前日のチームを表示しています`;
    ...
    const rows=scheduleRows(date,league),oldTable=$("tableInput").value,oldMatch=$("matchInput").value;
  ...
  function getInput(){const scheduled=currentScheduleRow(),rows=getRows($("playerInputs"));return{...scheduled,rows,date:inputDate||today()}}
  ...
  function renderList(){
    ...
    $("matchList").innerHTML=items.length?items.map(({key,item})=>{const readOnly=isReadOnlyHistory(item),urlMissing=item.requiresUrl&&!item.url,status=item.requiresUrl?`<span class="status ${urlMissing?"status-warning":"status-done"}">✓ ${urlMissing?"URL未入力":"URL入力済み"}</span>`:readOnly?`<span class="status status-done">閲覧専用</span>`:"";return`<article class="match-card ${urlMissing?"needs-action":""}" data-card-key="${esc(key)}"><div class="match-card-main"><div><div class="match-name">${esc(matchLabel(item))}</div><div class="match-time">送信 ${esc(item.time)}${readOnly?"・成績は閲覧専用":""}</div></div>${status}</div>${scoreSummary(item)}<div class="match-card-actions">${item.requiresUrl?`<button class="url-action" data-key="${esc(key)}">${item.url?"URLを確認・変更":"URLを記入してください"}</button>`:""}</div></article>`}).join(""):`<div class="panel empty-list">この日に入力された試合はまだありません。</div>`;
    document.querySelectorAll(".match-card").forEach(card=>card.addEventListener("click",event=>{if(event.target.closest("button"))return;const item=data[card.dataset.cardKey];if(item&&!isReadOnlyHistory(item)&&isAdminMode())openEdit(card.dataset.cardKey)}));document.querySelectorAll(".url-action").forEach(button=>button.onclick=()=>openUrl(button.dataset.key));
  }
  ...
  function openUrl(key){selectedKey=key;const item=readStore()[key];$("urlTitle").textContent=matchLabel(item);$("urlMatchInfo").innerHTML=`<div class="match-summary">${item.league?`<strong>${esc(item.league)}</strong>`:""}<span>送信 ${esc(item.time)}</span></div>${detailRows(item)}`;$("matchUrl").value=item.url||"";$("urlMessage").hidden=true;show("urlView");if(!localStorage.getItem(URL_HELP_SEEN_KEY))$("urlHelpModal").hidden=false}
  ...
  $("backToInput").onclick=()=>show("inputView");$("submitScores").onclick=async()=>{if(currentDaySubmissionLocked(pending.date)){alert("当日分の成績は21時以降に送信できます。");return}const button=$("submitScores"),key=rowKey(pending);button.disabled=true;try{const data=readStore();if(DEMO_MODE){data[key]={...pending,id:key,time:new Date().toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"}),url:""};writeStore(data)}else{const result=await apiRequest({action:"submit",id:key,data:pending});data[key]=result.entry;writeStore(data)}$("successModal").hidden=false}catch(error){alert(error.message)}finally{button.disabled=false}};$("closeSuccess").onclick=()=>{$("successModal").hidden=true;$("listDate").value=pending.date;if(pending.league)$("listLeague").value=pending.league;refreshListFilters();show("listView")};
  ...
  $("saveUrl").onclick=async()=>{const value=$("matchUrl").value.trim();if(!isReplayUrl(value)){$("urlMessage").textContent="試合リプレイのURLではありません。";$("urlMessage").hidden=false;return}try{if(!DEMO_MODE)await apiRequest({action:"url",id:selectedKey,url:value});const data=readStore();data[selectedKey].url=value;writeStore(data);$("urlMessage").hidden=true;refreshListFilters();show("listView")}catch(error){$("urlMessage").textContent=error.message;$("urlMessage").hidden=false}};$("backToListTop").onclick=$("backToListBottom").onclick=()=>{refreshListFilters();show("listView")};
  ...
  $("closeKeypad").onclick=()=>{$("scoreKeypad").hidden=true;activeScoreInput=null};$("scoreKeypad").onclick=event=>{if(event.target===$("scoreKeypad"))$("closeKeypad").click()};$("openUrlHelp").onclick=$("openEditUrlHelp").onclick=()=>$("urlHelpModal").hidden=false;$("closeUrlHelp").onclick=()=>{localStorage.setItem(URL_HELP_SEEN_KEY,"1");$("urlHelpModal").hidden=true};
  ...
  function startInputDateTimer(){setInterval(()=>{if(!autoInputDate)return;const next=timedInputDate();if(next===inputDate)return;inputDate=next;refreshInputSelectors()},30000)}
  async function init(){sessionStorage.removeItem("hldbScoreAdminMode");await loadReferenceData();inputDate=DEMO_MODE?(scheduledDates().find(date=>date>=today())||scheduledDates()[0]||today()):timedInputDate();if(!DEMO_MODE){try{await loadServerEntries()}catch(error){console.warn(error)}}else{document.title="ハンドレッドリーグ 成績入力｜操作練習";$("demoModeBanner").hidden=false;$("adminModeButton").hidden=true;$("successModal").querySelector("h2").textContent="練習入力が完了しました";$("successModal").querySelector("p").textContent="練習データのため、スプレッドシートには送信されません。"}$("listDate").value=inputDate;updateAdminButton();setupInputPreferences();if(!DEMO_MODE)startInputDateTimer()}
  init();
})();
