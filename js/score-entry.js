(function(){
  ...
  function refreshUrlButtons(url){const saved=Boolean(url);$("saveUrl").textContent=saved?"URLを変更する":"URLを保存する";$("openSavedUrl").hidden=!saved;$("openSavedUrl").dataset.url=saved?url:"";$("saveUrl").parentElement.classList.toggle("single",!saved)}
  function openUrl(key){selectedKey=key;const item=readStore()[key];$("urlTitle").textContent=matchLabel(item);$("urlMatchInfo").innerHTML=`<div class="match-summary">${item.league?`<strong>${esc(item.league)}</strong>`:""}<span>送信 ${esc(item.time)}</span></div>${detailRows(item)}`;$("matchUrl").value=item.url||"";refreshUrlButtons(item.url||"");$("urlMessage").hidden=true;show("urlView");if(!localStorage.getItem(URL_HELP_SEEN_KEY))$("urlHelpModal").hidden=false}
  ...
  $("saveUrl").onclick=async()=>{const value=$("matchUrl").value.trim();if(!isReplayUrl(value)){$("urlMessage").textContent="試合リプレイのURLではありません。";$("urlMessage").hidden=false;return}try{if(!DEMO_MODE)await apiRequest({action:"url",id:selectedKey,url:value});const data=readStore();data[selectedKey].url=value;writeStore(data);refreshUrlButtons(value);$("urlMessage").hidden=true;refreshListFilters();show("listView")}catch(error){$("urlMessage").textContent=error.message;$("urlMessage").hidden=false}};$("openSavedUrl").onclick=()=>{const value=$("openSavedUrl").dataset.url;if(isReplayUrl(value))window.open(value,"_blank","noopener,noreferrer")};$("backToListTop").onclick=$("backToListBottom").onclick=()=>{refreshListFilters();show("listView")};
